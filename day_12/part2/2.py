...
def main():
    ...
    paths = []
    start_points = [(y, x) for y, j in grid.items() for x, height in j.items() if height == ord('a')]
    for start in start_points:
        node = end
        backpath = [end]
        path = []
        while node != start and node in parents:
    ...
    print(f"Answer: {len(paths[0]) - 1}")

    for y, i in grid.items():
        for x, j in i.items():
            tup = (y, x)
            print(f"{Style.BRIGHT if (tup in paths[0]) else Style.DIM} {j:>3}{Style.RESET_ALL}", end='')
        print()
    print()
